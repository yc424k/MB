...
int InitSerialComm(const std::string &port_name, int baud_rate)
{
    ...
    ser_driver = std::make_unique<drivers::serial_driver::SerialDriver>();
    try
    {
        ser_driver->init_port(port_name, drivers::serial_driver::FlowControl::NONE, drivers::serial_driver::Parity::NONE, drivers::serial_driver::StopBits::ONE);
        ser_driver->set_baud_rate(baud_rate);
        ser_driver->open();
    }
...
int MdReceiveProc()
{
    ...
    case PID_IO_MONITOR:
    {
        RCLCPP_INFO(logger, "RCV: PID_IO_MONITOR: %d, %d", byRcvDataSize, (int)sizeof(PID_IO_MONITOR_t));
        memcpy((char *)&curr_pid_io_monitor, (char *)pRcvData, sizeof(PID_IO_MONITOR_t));
        RCLCPP_INFO(logger, "Inout voltage: %d", curr_pid_io_monitor.input_voltage);
        break;
    }
    case PID_ROBOT_MONITOR2:
    {
        RCLCPP_INFO(logger, "RCV: PID_ROBOT_MONITOR2: %d, %d", byRcvDataSize, (int)sizeof(PID_ROBOT_MONITOR2_t));
        memcpy((char *)&curr_pid_robot_monitor2, (char *)pRcvData, sizeof(PID_ROBOT_MONITOR2_t));
        RCLCPP_INFO(logger, "Inout voltage: %d", curr_pid_robot_monitor2.sVoltIn);
        break;
    }
    case PID_PNT_MAIN_DATA:
        if (byRcvDataSize == sizeof(PID_PNT_MAIN_DATA_t))
        {
            RCLCPP_DEBUG(logger, "RCV: PID_PNT_MAIN_DATA");
    ...
    case PID_ROBOT_MONITOR:
        ...
        {
            RCLCPP_DEBUG(logger, "RCV: PID_ROBOT_MONITOR_t");
            pid_response_receive_count++;
            pid_request_cmd_vel_count = 2;
